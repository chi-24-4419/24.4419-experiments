...
from imlmlib.mle_utils import compute_summary_statistics_estimation
from imlmlib.information import compute_observed_information
...
for filename in tqdm(files):
    with open(filename, "r") as _file:
        ...
        agg_data, df = compute_summary_statistics_estimation(estimated_parameters, subsample_sequence, TRUE_VALUE, ax = None,      bias_kwargs=None, std_kwargs=None,)
...
axs[0].set_xlabel('observed information')
axs[0].set_ylabel('std')
axs[1].set_xlabel('observed information')
axs[1].set_ylabel('std')
axs[0].set_title('std of alpha')
axs[1].set_title('std of beta')

seaborn.scatterplot(x=information_container, y=alpha_std_container, hue = hue_container, ax = axs[0])
seaborn.scatterplot(x=information_container, y=beta_std_container, hue = hue_container, ax = axs[1])
plt.tight_layout()
plt.show()