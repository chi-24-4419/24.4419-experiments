...
def diff_eval(ALPHA, intertrial_time_A, intetrial_timeB, interblock_time, condition_names, *args):
    data_C, deltas_C, k_repetition_C = simulator_block_ef(ALPHA, *args, intertrial_time_A, interblock_time)
    block_average_mean_C = numpy.mean(data_C, axis = 2)
    
    data_A, deltas_A, k_repetition_A = simulator_block_ef(ALPHA, *args, intetrial_timeB, interblock_time)
...
if __name__ == '__main__':
    ...
    intertrial_time_B = 5
    df1, data_A, deltas_A, k_repetition_A, data_B, deltas_B, k_repetition_B, mean_A_recall, mean_B_recall = diff_eval(ALPHA, intertrial_time_A, intertrial_time_B, interblock_time, ['A', 'B'] ,BETA, SIGMA, repet_trials, nitems, pop_size, replications)
    ...
    # A
    recall, deltas, krepet = data_A[:,RECALL_BLOCKS,:], deltas_A[:,RECALL_BLOCKS,:], k_repetition_A[:,RECALL_BLOCKS,:]
    ax, inf, covar = ML_plot_CE(recall, deltas, krepet, axs[1], colors=["#ff99bb", "#ff1a66"], plot_kwargs={"color": "#ff1a66", "marker": "D", "label": "A"})
    # B2
    recall, deltas, krepet = data_B[:,RECALL_BLOCKS,:], deltas_B[:,RECALL_BLOCKS,:], k_repetition_B[:,RECALL_BLOCKS,:]
    ax, inf, covar = ML_plot_CE(recall, deltas, krepet, axs[1], colors=["#ffd699", "#ffa31a"], plot_kwargs={"color": "#ffa31a", "marker": "D", "label": "B"})
