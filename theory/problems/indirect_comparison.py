from imlmlib.mem_utils import experiment, GaussianPopulation
...
if __name__ == '__main__':
    ...
    df1, data_C1, deltas_C1, k_repetition_C1, data_A1, deltas_A1, k_repetition_A1 = diff_eval(ALPHA_C,ALPHA_A, intertrial_time,interblock_time, ['C', 'A'] ,BETA, SIGMA, repet_trials, nitems, pop_size, replications)

    ## == B vs C
    interblock_time=[0, 1000, 400, 400,  400,  86400,  1000, 400] # À la BodyLoci
    intertrial_time = 4

    df2, data_C2, deltas_C2, k_repetition_C2, data_B2, deltas_B2, k_repetition_B2 = diff_eval(ALPHA_C,ALPHA_B, intertrial_time,interblock_time, ['C', 'B'] ,BETA, SIGMA, repet_trials, nitems, pop_size, replications)
    ...
    seaborn.barplot(data = df2, x= 'block', y="block recall %", hue = 'Condition',  errorbar = 'se', ax = axs[1])
    


    ## ==== Model-based comparison
    # C1
    recall, deltas, krepet = data_C1[:,RECALL_BLOCKS,:], deltas_C1[:,RECALL_BLOCKS,:], k_repetition_C1[:,RECALL_BLOCKS,:]
    ax, inf, covar = ML_plot_CE(recall, deltas, krepet, axs[2], colors=["#B0E0E6", "#37a0ae"], plot_kwargs={"color": "#37a0ae", "marker": "D", "label": "C(1)"})
    # C2
    recall, deltas, krepet = data_C2[:,RECALL_BLOCKS,:], deltas_C2[:,RECALL_BLOCKS,:], k_repetition_C2[:,RECALL_BLOCKS,:]
    ax, inf, covar = ML_plot_CE(recall, deltas, krepet, axs[2], colors=["#99d6ff", "#0099ff"],  plot_kwargs={"color": "#0099ff", "marker": "D", "label": "C(2)"})
    # A1
    recall, deltas, krepet = data_A1[:,RECALL_BLOCKS,:], deltas_A1[:,RECALL_BLOCKS,:], k_repetition_A1[:,RECALL_BLOCKS,:]
    ax, inf, covar = ML_plot_CE(recall, deltas, krepet, axs[2], colors=["#ff99bb", "#ff1a66"], plot_kwargs={"color": "#ff1a66", "marker": "D", "label": "A(1)"})
    # B2
    recall, deltas, krepet = data_B2[:,RECALL_BLOCKS,:], deltas_B2[:,RECALL_BLOCKS,:], k_repetition_B2[:,RECALL_BLOCKS,:]
    ax, inf, covar = ML_plot_CE(recall, deltas, krepet, axs[2], colors=["#ffd699", "#ffa31a"], plot_kwargs={"color": "#ffa31a", "marker": "D", "label": "B(2)"})
